...
import { generateReplaySource, generateUnitTestSource, combineUnitTests, generateUnitTestSourceNoCallEvent } from "./codegen.js";
...
export function runReplay(traceFile: string, outDir: string, useTypeNames = false, ut = false, noCallEvent = false): void {
  const traceDir = path.dirname(traceFile);
  const index = noCallEvent ? {
    files: [],
    calls: {}
  } : ensureReplayIndex(traceDir);
  ...
  if (noCallEvent) {
    ...
    triples.forEach((triple) => {
      ...
      try {
        const des = generateUnitTestSourceNoCallEvent(triple, useTypeNames, inferTypedTriple);
